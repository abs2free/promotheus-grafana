...
export default function () {
  ...
  check(res5, {
    ...
    "response contains Internal Server Error": (r) =>
      r.body.includes("Internal Server Error"),
  });

  let res6 = http.get(
    "http://host.docker.internal:8080/get-user?param=not-found"
  );
  check(res6, {
    "status is 500": (r) => r.status === 500,
  ...
  let res8 = http.get(
    "http://host.docker.internal:8080/get-role?param=not-found"
  );
  check(res8, {
    "status is 500": (r) => r.status === 500,
    "response contains Not Found": (r) => r.body.includes("Not Found"),
